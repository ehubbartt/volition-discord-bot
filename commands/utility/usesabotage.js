    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (targetProgress.length === 0) {
                return interaction.editReply({
                    content: `Team **${targetTeamName}** hasn't started on their current tile yet. Nothing to sabotage!`
                });
            }

            const targetItem = targetProgress.find(p => p.item_name.toLowerCase() === itemName.toLowerCase());
            if (!targetItem) {
                const validItems = targetProgress.map(p => p.item_name).join(', ');
                return interaction.editReply({
                    content: `**${itemName}** is not being tracked for team **${targetTeamName}** on tile ${targetTeam.current_tile}.\n\nValid items: ${validItems}`
                });
            }
